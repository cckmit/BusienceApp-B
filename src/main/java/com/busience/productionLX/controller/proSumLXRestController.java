...
public class proSumLXRestController {
	 ...
	 * public List<PRODUCTION_INFO_TBL> proMachineSumSelect2(HttpServletRequest
	 * request) throws ParseException, SQLException,
	 * org.json.simple.parser.ParseException { String startDate =
	 * request.getParameter("startDate"); String endDate =
	 * request.getParameter("endDate"); String EQUIPMENT_INFO_CODE =
	 * request.getParameter("EQUIPMENT_INFO_CODE"); String EQUIPMENT_INFO_NAME =
	 * request.getParameter("EQUIPMENT_INFO_NAME");
	 * 
	 ...
	 * ;
	 * 
	 * String where = " and A.PRODUCTION_Date between '" + startDate +
	 * " 00:00:00' and '" + endDate + " 23:59:59'\r\n";
	 * 
	 * // �����ڵ� �˻� if (EQUIPMENT_INFO_CODE != null) { where +=
	 * " and A.PRODUCTION_Equipment_Code like '%" + EQUIPMENT_INFO_CODE + "%'" +
	 * " and C.EQUIPMENT_INFO_NAME like '%" + EQUIPMENT_INFO_NAME + "%'"; } //
	 * �����? �˻� else { where += " and (C.EQUIPMENT_INFO_NAME like '%" +
	 * EQUIPMENT_INFO_NAME + "%'" + " OR A.PRODUCTION_Equipment_Code like '%" +
	 * EQUIPMENT_INFO_NAME + "%')"; } sql += where;
	 * 
	 * sql += " group by A.PRODUCTION_WorkOrder_ONo with rollup";
	 * 
	 * System.out.println(sql); Connection conn = dataSource.getConnection();
	 * PreparedStatement pstmt = conn.prepareStatement(sql); ResultSet rs =
	 * pstmt.executeQuery();
	 * 
	 * int i = 0; List<PRODUCTION_INFO_TBL> list = new
	 * ArrayList<PRODUCTION_INFO_TBL>();
	 * 
	 * while (rs.next()) { if (rs.getString("PRODUCTION_WorkOrder_ONo") == null) {
	 ...
	 * list.add(data1);
	 * 
	 * } else { PRODUCTION_INFO_TBL data1 = new PRODUCTION_INFO_TBL(); i++;
	 * data1.setId(i);
	 ...
	 * ; data1.setPRODUCTION_EQUIPMENT_INFO_NAME(rs.getString(
	 ...
	 * data1.setPRODUCTION_Date(rs.getString("PRODUCTION_Date")); list.add(data1); }
	 * }
	 * 
	 * rs.close(); pstmt.close(); pstmt.close(); conn.close(); return list; }
	 */
