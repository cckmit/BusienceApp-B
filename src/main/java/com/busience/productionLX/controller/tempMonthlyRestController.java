...
@RestController("tempMonthlyRestController")
@RequestMapping("tempMonthlyRest")
public class tempMonthlyRestController {
	
	@Autowired
	JdbcTemplate jdbctemplate;

	@RequestMapping(value = "/History_Monthly_List", method = RequestMethod.GET)
	public List<Equip_Temperature_History> History_Daily_List(HttpServletRequest request)
	{
		...
		String sql = "SELECT\r\n"
				...
				+ "			DATE_FORMAT(Temp_Time, '%Y-%m') Time\r\n"
				+ "FROM		Equip_Temperature_History\r\n"
				+ "WHERE		Temp_Time BETWEEN ? AND ?\r\n"
				+ "GROUP BY DATE_FORMAT(Temp_Time, '%Y-%m')";
