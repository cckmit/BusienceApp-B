...
public interface DTL_Mapper {
	...
	// 세부 테이블 정보 조회
	@Select("select * from DTL_TBL where NEW_TBL_CODE= #{NEW_TBL_CODE} and CHILD_TBL_USE_STATUS='true' order by CHILD_TBL_NUM*1")
	List<DTL_TBL> findByCode(@Param("NEW_TBL_CODE") String code);
	
	// 현재 연월 조회 및 익월 조회 - CHILD_TBL_RMARK
	@Select("select * from DTL_TBL where NEW_TBL_CODE= #{NEW_TBL_CODE} and CHILD_TBL_NUM= #{CHILD_TBL_NUM} and CHILD_TBL_USE_STATUS='true' order by CHILD_TBL_NUM*1")
	...
	// 부서명 조회하기 위한 쿼리
	@Select("select * from DTL_TBL where NEW_TBL_CODE= #{NEW_TBL_CODE} and CHILD_TBL_NUM > 0 and CHILD_TBL_USE_STATUS='true' order by CHILD_TBL_NUM*1")
