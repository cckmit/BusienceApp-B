...
var matOutputSalesCustomerTable = new Tabulator("#matOutputSalesCustomerTable", {
...
$("#MOSL_SearchBtn").click(function() {
	matOutputSalesListTable.clearData();
	MOSL_Search();
})

function MOSL_Search() {
	var thisMonth = $("#selectedMonth").val() + "-01";
	var nextMontth = new Date($("#selectedMonth").val() + "-01");
	nextMontth = new Date(nextMontth.setMonth(nextMontth.getMonth() + 1)).toISOString().substring(0, 10);
	...
	matOutputSalesCustomerTable.setData("matOutputReportLXRest/MOSL_Search", datas)
	.then(function(){
		UseBtn();
	})
}

var matOutputSalesListTable = new Tabulator("#matOutputSalesListTable", {
	layoutColumnsOnNewData: true,
	//Sub Total 색상
	rowFormatter: function(row) {
