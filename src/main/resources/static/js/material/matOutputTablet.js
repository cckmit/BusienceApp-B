...
{
	$.get("/tablet/matOutputTabletRest/Out_Clsfc",data,function(data2){
		console.log(data2);
...
function search(){
	...
	$.ajax({
		method : "GET",
		dataType : "json",
		url : "../matOutputReportRest/tablet/MO_ListView?data="+ encodeURI(JSON.stringify(data)),
...
function qtyselectFun()
{
	...
	$.get("/tablet/matOutputTabletRest/Current_Save",data,function(idata){
...
document.getElementById("okbtn").onclick = function(){
	...
	$.get("/tablet/matOutputTabletRest/MOM_Save",data,function(){
		location.reload();
	});
}
...
function list_select(){
	...
	if(flag === "one"){
		...
		$.get("/tablet/matOutputTabletRest/Current_List",data,function(data){
			console.log(data);
...
var itemManageTable = new Tabulator("#itemManageTable",	{
		...
		rowDblClick: function(e, row) {
			...
			$.get("/tablet/matOutputTabletRest/Current_Save?code="+row.getData().product_ITEM_CODE,function(data){
...
var itemManageTable3 = new Tabulator("#itemManageTable3",	{
		ajaxURL:"/tablet/matOutputTabletRest/Out_List",
		layout:"fitDataStretch",
		//페이징
		layoutColumnsOnNewData : true,
		headerFilterPlaceholder: null,
