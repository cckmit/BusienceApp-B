<%@page import="com.busience.standard.dto.DEFECT_INFO_TBL"%>
			...
			%>
					var m00<%=i%>_table = new Tabulator("#m00<%=i%>_table", {
					    ...
					    columns:[
							...
							<%
								List<DEFECT_INFO_TBL> list = (List<DEFECT_INFO_TBL>)request.getAttribute("defect_list");
								
								for(int h=0;h<list.size();h++)
								{
									out.println(",{title:'"+list.get(h).getDEFECT_NAME()+"',field:'"+list.get(h).getDEFECT_CODE()+"', headerHozAlign: 'center', headerSort:false, align: 'right'}");
								}
							%>
					    ],
					});
